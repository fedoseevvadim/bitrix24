BX.ready(function () {
    ...
    var pageTask = "/company\/personal\/user\/" + user + "\/tasks\/"; // Переменная нужна для кнопки Добавить заявку, проверка идет на открытие этой страницыы
    ...
    if ( task ) { // will return true
        ...
        let checkExist = setInterval(function() {
            ...
            //$('.task-detail-sidebar-item task-detail-sidebar-item-robot').remove(); // убираем Автоматизация роботы (из правой части) // х.з почему не убирает
            ...
            // Убираем пользовательские поля
            // if ( properties !== null ) {
            //
            //     for ( let i = 0; i < properties.length; i++ ) {
            //
            //         if ( properties[i].innerHTML === 'Номер заявки' ) {
            //             properties[i].remove();
            //             propertiesVal[i].remove();
            //         }
            //
            //     }
            //
